...
* 外部账户： 拥有公私钥对，可发送交易的用户。支持用户的身份标识(identity)。
* 合约账户： 拥有相关的代码(code)及存储(storage)。

目前权限管理针对外部账户进行细粒度管理。CITA默认集成了superAdmin账户，拥有权限管理涉及到的所有权限。在CITA启动前可以对superAdmin进行公钥配置。由用户生成的外部账户，在CITA系统中没有任何权限，需要superAdmin对其进行授权。CITA未做任何配置时，权限管理默认未开启，即所有用户都有所有权限，可以直接对其他账号进行制授权。
...
* 修改x0000000000000000000000000000000031415926的第一项打开权限管理，由false修改为true。通过bin/admintool.sh生成各节点。

## 权限管理概述
权限(permission)在此系统中的定义为多个资源(resource)的集合，其中资源(resource)为一个合约地址及一个函数签名。
下面以实例方式介绍如何使用权限管理。用户生成普通账户，由superAdmin账户对其进行授权，实现权限管理。使用superAdmin的私钥调用的接口由管理员执行，使用用户john的私钥由用户john执行。

### 生成普通账户
用户可以自己生成公私钥对。也可以使用CITA提供的工具create_key_addr生成。例如用户john通过工具生成如下密钥。
根据输入的字符串"john"使用`./bin/create_key_addr /tmp/privKey "john"`命令生成账户地址，/tmp/privKey文件内容为工具生成的私钥，“john”为当前目录下生成以"john"命名的文件记录账户地址（公钥）。
...
其中--privkey为用户john的账号私钥，--code为Advance.sol的字节码。
status为OK，默认配置时表示合约已经发送到CITA链上。但此时我们已修改了默认配置，打开权限管理。虽然status为Ok，但是通过获得回执并没有合约地址(contractAddress:null)、并且errorMessage:"No contract permission."，表示此账户还未获得部署合约的权限。回执信息如下：
...
2.由于用户john的账户没有部署权限，需要通过superAdmin对其授createContract创建合约权限。
...
其中--to为权限管理的系统合约地址，--code前0x0f5aa9f3为系统合约的setAuthorization接口，其后紧跟需要授权地址和所授权限。0x2表示createContract创建合约权限。权限授予成功后用户john重做第一步，成功获得合约地址，表示合约部署成功。成功回执信息如下：
...
回执errorMessage返回值为null，此时Advance合约由用户john成功部署到CITA链上。但是此时的合约还没有用户能够使用(superAdmin除外)，用户john需要调用Advance合约还需要superAdmin用户给予授权。

### 生成新的权限
通过生成新的权限功能，使用户john能够使用Advance合约。

1.通过superAdmin授予用户john的账户newPermission权限，回执中errorMessage返回值为null，表示成功生成新权限。

```shell
######调用系统合约的newPermission生成新的权限
$python make_tx.py --privkey "993ef0853d7bf1f4c2977457b50ea6b5f8bc2fd829e3ca3e19f6081ddabb07e9" --to "00000000000000000000000000000000013241b2" --code ""
$python send_tx.py
$python get_receipt.py
{
  "contractAddress": null,
...
* --to 为权限管理的合约地址
* --code 前0xfc4a089c为newPermission接口hash值，后面为接口输入的参数
  ("Advance_function",["0x47113fea5720d201b31ecf82a7da5ea3ed150255"],["0x4f2be91f"])，
  0x47113fea5720d201b31ecf82a7da5ea3ed150255为部署合约时生成的合约地址，0x4f2be91f为合约的add()方法。
  执行成功从get_receipt.py回执logsi[0].address获得新的权限地址"0x3682affa243cb9536cb6989307c54e388198e709"。目前newPermission方法新生成的权限地址记录在logs[0].address中。

2.给用户john授予新的权限使其可以调用Advance.sol合约中的add方法。
在未授权前用户john调用合约的方法会报errorMessage:"No transaction permission."错误，说明用户john没有获得发交易权限，此时并不能调用合约的add()方法。

```shell
#####用户john调用Advance合约的add方法
$python make_tx.py --privkey "a71f68fd5f0a64c0a66737357ec6e491c5bab8e001f8d7116252c22a9a4f03b4" --to "47113fea5720d201b31ecf82a7da5ea3ed150255" --code "4f2be91f"
$python send_tx.py
$python get_receipt.py
{
  "contractAddress": null,
  "cumulativeGasUsed": "0x0",
  "logs": [],
  "blockHash": "0xde40afc3a771d8b6a6c29ad3ebd1fa2e8d5090d00c72ef665c8b352d947179aa",
  "transactionHash": "0x78dcd8da73affa585282437c27e22be7cceb14e09d3ce40974aee94122492ce3",
  "root": null,
...
首先需要先获得发送交易(sendTx)权限，然后使用newPermission生成新的权限地址，通过superAdmin调用setAuthorization系统合约方法给john授权可以调用合约add()方法：

```shell
######调用系统合约setAuthorization对用户john授予sendTx权权限,sendTx对应的ID为0x01,ID作为setAuthorization的第二个参数
...
######调用系统合约setAuthorization对用户john授予newPermissionu新生成的权限
$python make_tx.py --privkey "993ef0853d7bf1f4c2977457b50ea6b5f8bc2fd829e3ca3e19f6081ddabb07e9" --to "00000000000000000000000000000000013241b2" --code "0f5aa9f30000000000000000000000006212dd3506a68d6ec231177c6cb9c46dcfd431900000000000000000000000003682affa243cb9536cb6989307c54e388198e709"
$python send_tx.py
$python get_receipt.py
```

用户john获得权限后就能使用Advance.add方法，errorMessage:null表示成功调用。
...
其中params.to参数，为queryPermissions所在系统合约地址；params.data为调用eth_call的输入参数，由queryPermissions的hash和queryPermissions参数的hash组成。
