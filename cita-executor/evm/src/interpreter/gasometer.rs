...
impl<Gas: CostType> Gasometer<Gas> {
    ...
    pub fn requirements(
        ...
        let cost = match instruction {
            ...
            instructions::SHA3 => {
                ...
                let words = w >> 5;
                let gas =
                    Gas::from(schedule.sha3_gas) + (Gas::from(schedule.sha3_word_gas) * words);
    ...
    fn mem_gas_cost(
        ...
        let req_mem_size_rounded =
            (overflowing!(mem_size.overflow_add(Gas::from(31 as usize))) >> 5) << 5;
