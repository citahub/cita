...
impl Executor {
    ...
    pub fn write_batch(&self, block: ClosedBlock) {
        ...
        // Insert [height : hash]
        let height_key = indexes::BlockNumber2Hash(height).get_index();
        self.db
    ...
    fn block_hash(&self, number: BlockNumber) -> Option<H256> {
        let height_key = indexes::BlockNumber2Hash(number).get_index();
        self.db
...
impl<'a> BloomGroupDatabase for Executor {
    fn blooms_at(&self, position: &GroupPosition) -> Option<BloomGroup> {
        let position = indexes::LogGroupPosition::from(position.clone());
