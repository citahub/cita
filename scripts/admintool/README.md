...
```shell
usage: ./scripts/admintool/admintool.sh -a admin_id -l ip_list -n consensus_name -m crypto_method -t
option:
...
-Q node id, use to create a new node, usually use with -l, -l must list all node ip

-T timestamp of genesis
   default value is current timestamp

-C chain_id of current network
   default value is random.randint(0, 2**29)

-E economical model (enum: quota, charge)
   default value is 'quota'

-A authorities file of current network
   default generated by create_key_addr for n nodes

-i current chain id for chain management, a unique id

-p parent chain id, the unique id of the parent chain

-P parent chain nodes, the parent chain's consensus nodes

-f prefix of node directories

-D development mode, the port for HTTP, WebSocket, Kafka, GRPC ports will be increased automatically for nodes.
   default port is 'true'

-S specify a step if do not want generate all files in one step.
   options: init-key | init-config.
```
