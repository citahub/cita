...
## 1. 复制 CITA 到自己的仓库中（Fork repository）
...
从自己的 Github 主页中进入 CITA 仓库，点击右侧的 <kbd>Clone or download</kbd> 按钮，复制 HTTPS 链接。
...
## 5. 创建并提交 PR（Create pull request）

在同步完成后，即可通过 <kbd>Create pull request</kbd> 按钮将该分支发送给该项目的维护者等待被合并。如果你的 Pull Request 修复了在 Issues 中描述的问题，可以使用 [special magic keyword](https://help.github.com/articles/closing-issues-via-commit-messages/) 引用该 Issue 为参考
