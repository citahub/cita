...
use connection::{Connection, broadcast};
use libproto::*;
...
impl MyHandler {
    pub fn new(con: Connection, puber: Pub) -> Self {
        MyHandler {
            con: con,
            puber: puber,
        }
...
fn handle_rpc(con: &Connection, puber: &mut Pub, payload: &[u8]) {
    if let Ok(msg) = parse_from_bytes::<communication::Message>(payload) {
        ...
        if cid == cmd_id(submodules::JSON_RPC, topics::REQUEST) && t == MsgType::REQUEST {
            ...
            if ts.has_peercount() {
                let peercount = con.peers_pair
                    .iter()
                    .filter(|x| x.2.as_ref().read().is_some())
                    .count();
