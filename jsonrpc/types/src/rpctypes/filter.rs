...
use state::filter::Filter as EthFilter;
use state::ids::BlockId;
use rpctypes::block_number::BlockNumber;
use serde_types::hash::{H256, Address};

/// Variadic value
#[derive(Debug, PartialEq, Eq, Clone, Hash, Serialize)]
#[serde(untagged)]
pub enum VariadicValue<T> where T: DeserializeOwned + Serialize {
	/// Single
	Single(T),
	/// List
	Multiple(Vec<T>),
...
impl<'de, T> Deserialize<'de> for VariadicValue<T> where T: DeserializeOwned + Serialize {
...
mod tests {
	...
	fn filter_deserialization2() {
		...
		assert_eq!(deserialized, Filter {
			...
			to_block: None,
			address: None,
			topics: Some(vec![VariadicValue::Single(H256::from_str("8fb1356be6b2a4e49ee94447eb9dcb8783f51c41dcddfe7919f945017d163bf3").unwrap().into())]),
			limit: None,
		});
	}
