...
impl PubSub {
    pub fn new() -> Self {
        let amqp_url = std::env::var(AMQP_URL)
            .expect(format!("{} must be set", AMQP_URL).as_str());
        ...
        where T: Consumer + 'static
    {
        let mut channel = self.session
            .open_channel(self.channel_no)
            .ok()
            .expect("Can't open channel");
        let _ = channel.basic_prefetch(10);
        self.channel_no += 1;
        channel
            .exchange_declare("cita",
                              "topic",
                              false,
                              true,
                              false,
...
mod test {
    ...
    fn basics() {
        ...
        net_pubsub.start_sub("network",
                             vec!["chain.newtx", "chain.newblk"],
                             MyHandler::new());
        chain_pubsub.start_sub("chain",
