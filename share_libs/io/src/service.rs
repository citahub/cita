    ...
    Message: Send + Clone + Sized,
{
    ...
    AddHandler { handler: Arc<IoHandler<Message> + Send>, },
    RemoveHandler { handler_id: HandlerId },
    AddTimer {
    ...
    Message: Send + Clone + Sync + 'static,
{
    ...
    pub fn unregister_handler(&self) -> Result<(), IoError> {
        self.channel.send_io(IoMessage::RemoveHandler { handler_id: self.handler })?;
        Ok(())
    }
}
    ...
    Message: Send + Sync + Clone + 'static,
{
    ...
    pub fn start(event_loop: &mut EventLoop<IoManager<Message>>, handlers: Arc<RwLock<Slab<Arc<IoHandler<Message>>, HandlerId>>>) -> Result<(), IoError> {
        let (worker, stealer) = chase_lev::deque();
        let num_workers = 4;
        let work_ready_mutex = Arc::new(SMutex::new(()));
        let work_ready = Arc::new(SCondvar::new());
        let workers = (0..num_workers)
            .map(|i| Worker::new(i, stealer.clone(), IoChannel::new(event_loop.channel(), Arc::downgrade(&handlers)), work_ready.clone(), work_ready_mutex.clone()))
            .collect();
        ...
        if let Some(handler) = self.handlers.read().get(handler_index) {
            ...
            if let Some(timer) = maybe_timer {
                if timer.once {
                    ...
                    event_loop.timeout(token, Duration::from_millis(timer.delay))
    ...
    fn notify(&mut self, event_loop: &mut EventLoop<Self>, msg: Self::Message) {
        match msg {
            ...
            IoMessage::AddTimer { handler_id, token, delay, once } => {
                ...
                let timeout = event_loop.timeout(Token(timer_id), Duration::from_millis(delay))
                                        .expect("Error registering user timer");
                self.timers.write().insert(timer_id,
                                           UserTimer {
                                               delay: delay,
                                               timeout: timeout,
                                               once: once,
    ...
    Message: Send + Sync + Clone + 'static,
{
    ...
    pub fn start() -> Result<IoService<Message>, IoError> {
        ...
        let thread = thread::spawn(move || { IoManager::<Message>::start(&mut event_loop, h).expect("Error starting IO service"); });
        Ok(IoService {
    ...
    pub fn register_handler(&self, handler: Arc<IoHandler<Message> + Send>) -> Result<(), IoError> {
        self.host_channel.lock().send(IoMessage::AddHandler { handler: handler })?;
        Ok(())
    }
