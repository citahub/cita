...
use externalities::*;
use trace::{FlatTrace, Tracer, NoopTracer, ExecutiveTracer, VMTrace, VMTracer, ExecutiveVMTracer,
            NoopVMTracer};
use transaction::eth_transaction::{Action, SignedTransaction};
use crossbeam;
...
impl<'a, B: 'a + StateBackend> Executive<'a, B> {
        ...
        if let Some(contract) = self.engine.get_native_contract(&params.code_address) {
            ...
            if cost <= params.gas {
                ...
                let mut subvmtracer = vm_tracer.prepare_subtrace(params.code.as_ref().expect(
                    "scope is conditional on params.code.is_some(); qed",
                ));
    ...
    fn finalize(
        ...
        let suicide_refunds = U256::from(schedule.suicide_refund_gas) *
...
mod tests {
    ...
    use std::sync::Arc;
    use super::*;
    use util::{H256, U256, Address, Uint, FromHex, FromStr};
    use action_params::{ActionParams, ActionValue};
    use env_info::EnvInfo;
    ...
    fn test_call_contract() {
        ...
        let mut state = get_temp_state();
        state
            .init_code(&contract_addr, contract_code.clone())
            .unwrap();
        let mut params = ActionParams::default();
