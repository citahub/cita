...
use trace::FlatTrace;
use receipt::Receipt;
use transaction::eth_transaction::SignedTransaction;
...
impl<B: Backend> State<B> {
    ...
    fn note_cache(&self, address: &Address) {
        if let Some(ref mut checkpoint) = self.checkpoints.borrow_mut().last_mut() {
            if !checkpoint.contains_key(address) {
                checkpoint.insert(
                    address.clone(),
                    self.cache.borrow().get(address).map(
                        AccountEntry::clone_dirty,
                    ),
                );
            }
        }
    ...
    pub fn storage_root(&self, a: &Address) -> trie::Result<Option<H256>> {
        self.ensure_cached(a, RequireCache::None, true, |a| {
            a.as_ref().and_then(
                |account| account.storage_root().cloned(),
            )
        })
    }
        ...
        // check local cache first without updating
        {
            ...
            if let Some(ref mut acc) = local_account {
                if let Some(ref account) = acc.account {
                    let account_db = self.factories.accountdb.readonly(
                        self.db.as_hashdb(),
    ...
    pub fn code_hash(&self, a: &Address) -> trie::Result<H256> {
        self.ensure_cached(a, RequireCache::None, true, |a| {
            a.as_ref().map_or(SHA3_EMPTY, |a| a.code_hash())
        })
    }
    ...
    pub fn apply(
        ...
        let e = Executive::new(self, env_info, engine, &vm_factory)
            .transact(t, options)?;

        // TODO uncomment once to_pod() works correctly.
        //		trace!("Applied transaction. Diff:\n{}\n", state_diff::diff_pod(&old, &self.to_pod()));
        self.commit()?;
        let state_root = Some(self.root().clone());
        ...
        F: Fn(Option<&Account>) -> U,
    {
        ...
        if let Some(ref mut maybe_acc) = self.cache.borrow_mut().get_mut(a) {
            if let Some(ref mut account) = maybe_acc.account {
                let accountdb = self.factories.accountdb.readonly(
                    self.db.as_hashdb(),
                    account.address_hash(a),
                );
        ...
        let db = self.factories.trie.readonly(
            self.db.as_hashdb(),
            &self.root,
        )?;
        ...
        G: FnOnce(&mut Account),
    {
        ...
        Ok(RefMut::map(self.cache.borrow_mut(), |c| {
            let mut entry = c.get_mut(a).expect(
                "entry known to exist in the cache; qed",
            );
...
mod tests {
    ...
    use transaction::eth_transaction::UnverifiedTransaction;
    ...
    fn should_apply_create_transaction() {
        ...
        // 1) content = (code, nonce)
        let mut content = blockchain::Content::new();
        content.set_nonce(U256::from(0).to_hex());
        content.set_gas(1_000_000u64);
        content.set_data( ""
                               .from_hex()
                               .unwrap());
        content.set_value(0);


        // 2) tx = (from, content(code, nonce))
        let mut tx = blockchain::Transaction::new();
        tx.set_from(String::from("9cce34f7ab185c7aba1b7c8140d620b4bda941d6"));
        tx.set_to(String::from(""));
        tx.set_content(content.write_to_bytes().unwrap());
        tx.set_valid_until_block(0);

        // 3) tx = (from, content(code, nonce, signature))
        let privkey = cita_crypto::PrivKey::from(H256::from(
        ...
        stx.set_signature(
            cita_crypto::sign(&privkey, &tx.sha3().into())
                .unwrap()
                .to_vec(),
        );
        stx.set_crypto(blockchain::Crypto::SECP);
    ...
    fn remove_from_database() {
        ...
        let (root, db) = {
            let mut state = State::from_existing(db, root, U256::from(0u8), Default::default())
                .unwrap();
            assert_eq!(state.exists(&a).unwrap(), true);
            assert_eq!(state.nonce(&a).unwrap(), U256::from(1u64));
            state.kill_account(&a);
