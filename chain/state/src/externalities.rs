          ...
          B: StateBackend
{
        ...
        match *number < U256::from(self.env_info.number) &&
            ...
            true => {
                ...
                assert!(index < self.env_info.last_hashes.len() as u64,
                        format!("Inconsistent env_info, should contain at least {:?} last hashes",
                                index + 1));
    ...
    fn create(&mut self, gas: &U256, value: &U256, code: &[u8]) -> evm::ContractCreateResult {
        ...
        let mut ex = Executive::from_parent(self.state,
                                            self.env_info,
                                            self.engine,
                                            self.vm_factory,
                                            self.depth);
    ...
    {
        let handle_copy =
            |to: &mut Option<&mut Bytes>| { to.as_mut().map(|b| **b = data.to_owned()); };
        match self.output {
