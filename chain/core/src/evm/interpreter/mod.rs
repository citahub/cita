...
impl<Cost: CostType> Interpreter<Cost> {
    ...
    fn mem_written(instruction: Instruction, stack: &Stack<U256>) -> Option<(usize, usize)> {
        match instruction {
            ...
            instructions::CODECOPY => Some((stack.peek(0).low_u64() as usize, stack.peek(2).low_u64() as usize)),
    ...
    fn exec_instruction(&mut self, gas: Cost, params: &ActionParams, ext: &mut evm::Ext, instruction: Instruction, code: &mut CodeReader, stack: &mut Stack<U256>, provided: Option<Cost>) -> evm::Result<InstructionResult<Cost>> {
        match instruction {
            ...
            instructions::DELEGATECALL => {
                ...
                let value = if instruction == instructions::DELEGATECALL { None } else { Some(stack.pop_back()) };
