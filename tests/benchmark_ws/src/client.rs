...
#[bench]
fn bench_create_tx_time(b: &mut Bencher) {
    //cargo bench
    b.iter(|| create_tx_time())
}
...
mod test {
    ...
    fn test_create_tx_time() {
        //cargo test test_create_tx_time  -- --nocapture
        let start = time::precise_time_ns();
        println!("begin time. {} ", start);
        create_tx_time();
        let end = time::precise_time_ns();
        println!("begin time. {} ", end);
    ...
    #[test]
    fn test_create_uuid() {
        //cargo test test_create_uuid  -- --nocapture
        let start = time::precise_time_ns();
        println!("begin time. {} ", start);
        for _ in 1..30000 {
