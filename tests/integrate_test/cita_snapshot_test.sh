...
################################################################################
echo "6) Stop node-1 and node-2, so that node-0 cannot grow up via cita-consensus and cita-sync mechanisms"
...
for height in {10000,2,0}; do
    ...
    case height in
        0)     expect_height=${current_height};;
        2)     expect_height=2;;
        10000) expect_height=${current_height};;
    esac

    current_height=`get_height 0`
    if [ "${current_height}" = "${expect_height}" ]; then
        echo "FAILED: restore to ${expect_height}-th but chain is ${current_height}-th"
        exit 1
    fi
done
